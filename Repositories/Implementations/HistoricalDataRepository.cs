...
namespace Temperance.Constellations.Repositories.Interfaces.HistoricalData.Implementations
{
    public class HistoricalDataRepository : IHistoricalDataRepository
    {
        ...
        public async Task<bool> UpdateHistoricalPrices(List<PriceModel> prices, string symbol, string timeInterval)
        {
            ...
            var success = true;
            var batch = new List<PriceModel>();
            var securityId = await _securitiesOverviewRepository.GetSecurityId(symbol);

            foreach (var price in prices)
            {
                if (await DoesRecordExist(securityId, price.Timestamp, timeInterval))
                {
                    success &= await UpdatePriceRecord(price, timeInterval);
                    Console.WriteLine($"Successfully saved item: {price.Timestamp}, {price.Symbol}");
                }
                else
                {
                    batch.Add(price);
        ...
        public async Task<bool> InsertPriceBatch(int securityId, List<PriceModel> batch, string timeInterval)
        {
            try
            {
        ...
        private async Task<bool> UpdatePriceRecord(PriceModel price, string timeInterval)
        {
            using var connection = new SqlConnection(_connectionString);
            var updateQuery = @"
        ...
        private async Task<bool> InsertPriceRecord(int securityId, PriceModel price, string timeInterval)
        {
