...
namespace Temperance.Services.Trading.Strategies.Momentum
{
    public class MovingAverageCrossoverStrategy : ISingleAssetStrategy
    {
            ...
            MarketHealthScore marketHealth)
        {
            ...
            decimal kellySizedCapital = currentTotalEquity * kellyHalfFraction;

            decimal finalCapitalForSizing = Math.Min(kellySizedCapital, maxTradeAllocationInitialCapital);

            var atr = currentIndicatorValues["ATR"];
            if (atr == 0) return 0;

            decimal riskAmountPerShare = atr * _atrMultiplier;
            if (riskAmountPerShare == 0) return 0;

            decimal numShares = finalCapitalForSizing / riskAmountPerShare;
        ...
        public bool ShouldTakePartialProfit(Position position, in PriceModel currentBar, Dictionary<string, decimal> currentIndicatorValues) => false;
        public decimal GetAtrMultiplier() => 0;
        public decimal GetStdDevMultiplier() => 0;
        public decimal[] CalculateRSI(decimal[] prices, int period) => Array.Empty<decimal>();
