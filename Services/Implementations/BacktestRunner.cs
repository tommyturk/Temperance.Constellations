...
namespace Temperance.Constellations.Services.Implementations
{
    public class BacktestRunner : IBacktestRunner
    {
            ...
            List<PriceModel> dataWindow, Dictionary<string, decimal> indicators, MarketHealthScore? marketHealth, decimal initialCapital)
        {
            ...
            decimal totalEntryCost = await transactionCostService.GetSpreadCost(rawEntryPrice, quantity, sleeve.Symbol, sleeve.Interval, currentBar.Timestamp);
            ...
            Guid runId, string exitReason, IReadOnlyDictionary<string, decimal> indicators)
        {
            ...
            decimal effectiveExitPrice = await transactionCostService.CalculateExitCost(rawExitPrice, sleeve.CurrentPosition.Direction,
           ...
           Dictionary<string, decimal> currentIndicatorValues)
        {
            ...
            decimal effectiveExitPrice = await transactionCostService.CalculateExitCost(rawExitPrice, exitDirection, symbol, interval, exitBar.Timestamp);
            ...
            decimal spreadAndOtherCost = await transactionCostService.GetSpreadCost(rawExitPrice, currentPosition.Quantity, symbol, interval, exitBar.Timestamp);
                        ...
                        for (int i = backtestStartIndex; i < orderedData.Count; i++)
                        {
                            ...
                            if (signal != SignalDecision.Hold && currentPosition == null)
                            {
                                ...
                                if (allocationAmount > 0)
                                {
                                    ...
                                    var direction = (signal == SignalDecision.Buy) ? PositionDirection.Long : PositionDirection.Short;
                                    decimal effectiveEntryPrice = await transactionCostService.CalculateEntryCost(rawEntryPrice, signal, symbol, interval, currentBar.Timestamp);
                                    decimal totalEntryCost = await transactionCostService.GetSpreadCost(rawEntryPrice, quantity, symbol, interval, currentBar.Timestamp);
                                    decimal totalCashOutlay = (quantity * rawEntryPrice) + totalEntryCost;
