...
//        )
//        {
//            public Models.Trading.Position? CurrentPosition { get; set; }
...
//                {
//                    var positions = JsonConvert.DeserializeObject<IEnumerable<Models.Trading.Position>>(latestPortfolioState.OpenPositionsJson);
...
//            string parametersJson, decimal initialCapital,
//            List<PriceModel> priceData, bool useMocExit)
//        {
...
//        private async Task TryOpenPositionAsync(BacktestSleeve sleeve, PriceModel currentBar, SignalDecision signal, Guid runId,
//            List<PriceModel> dataWindow, Dictionary<string, decimal> indicators, MarketHealthScore? marketHealth, decimal initialCapital)
//        {
...
//            decimal allocationAmount = sleeve.Strategy.GetAllocationAmount(
//                in currentBar,
//                dataWindow,
//                indicators,
//                initialCapital * 0.02m,
...
//            decimal totalEntryCost = transactionCostService.GetSpreadCost(rawEntryPrice, quantity, sleeve.Symbol, sleeve.Interval, currentBar.Timestamp);
//            decimal totalCashOutlay = (quantity * rawEntryPrice) + totalEntryCost;
//            decimal commissionCost = await transactionCostService.CalculateCommissionCost(rawEntryPrice, quantity, sleeve.Symbol, sleeve.Interval, currentBar.Timestamp);
//            decimal slippageCost = await transactionCostService.CalculateSlippageCost(rawEntryPrice, quantity, direction, sleeve.Symbol, sleeve.Interval, currentBar.Timestamp);
//            decimal otherTransactionCost = await transactionCostService.CalculateOtherCost(rawEntryPrice, quantity, sleeve.Symbol, sleeve.Interval, currentBar.Timestamp);
...
//            decimal initialMae = 0.0m;
//            decimal initialMfe = 0.0m;
...
//            decimal entryAtr = indicators.GetValueOrDefault("ATR", 0);
//            if (entryAtr > 0)
//            {
...
//        private async Task<TradeSummary?> ClosePositionAsync(StrategySleeve sleeve, PriceModel currentBar,
//            Guid runId, string exitReason, IReadOnlyDictionary<string, decimal> indicators)
//        {
...
//            decimal effectiveExitPrice = transactionCostService.CalculateExitCost(rawExitPrice, sleeve.CurrentPosition.Direction,
//                sleeve.Symbol, sleeve.Interval, currentBar.Timestamp);
//            decimal exitTransactionCost = Math.Abs(rawExitPrice - effectiveExitPrice) * sleeve.CurrentPosition.Quantity;
...
//                else
//                    values[indicator.Key] = new decimal();
...
//            var allPrices = new List<PriceModel>();
...
//           TradeSummary activeTrade, Models.Trading.Position currentPosition, PriceModel exitBar,
//           string symbol, string interval, Guid runId, int rollingKellyLookbackTrades,
//           ConcurrentDictionary<string, decimal> symbolKellyHalfFractions, string exitReason,
...
//            decimal effectiveExitPrice = transactionCostService.CalculateExitCost(rawExitPrice, exitDirection, symbol, interval, exitBar.Timestamp);

//            decimal commissionCost = await transactionCostService.CalculateCommissionCost(rawExitPrice, currentPosition.Quantity, symbol, interval, exitBar.Timestamp);
//            decimal slippageCost = await transactionCostService.CalculateSlippageCost(rawExitPrice, currentPosition.Quantity, exitDirection, symbol, interval, exitBar.Timestamp);
//            decimal spreadAndOtherCost = transactionCostService.GetSpreadCost(rawExitPrice, currentPosition.Quantity, symbol, interval, exitBar.Timestamp);
//            decimal totalExitCost = commissionCost + slippageCost + spreadAndOtherCost;

//            decimal grossPnl = (exitDirection == PositionDirection.Long)
...
//            decimal totalTransactionCost = (activeTrade.TotalTransactionCost ?? 0) + totalExitCost;
//            decimal netPnl = grossPnl - totalTransactionCost;
...
//        private async Task<TradeSummary?> ClosePositionAsync(BacktestSleeve sleeve, PriceModel currentBar,
//            Guid runId, string exitReason, IReadOnlyDictionary<string, decimal> indicators)
//        {
//            if (sleeve.CurrentPosition == null || sleeve.ActiveTrade == null)
//            {
...
//                        Models.Trading.Position? currentPosition = null;
//                        TradeSummary? activeTrade = null;
//                        decimal currentSymbolKellyHalfFraction = symbolKellyHalfFractions.GetOrAdd($"{symbol}_{interval}", 0.01m);
...
//                            var currentIndicatorValues = new Dictionary<string, decimal>
//                            {
...
//                                decimal allocationAmount = strategyInstance.GetAllocationAmount(in currentBar, dataWindow, currentIndicatorValues, config.InitialCapital * 0.02m, _portfolioManager.GetTotalEquity(), currentSymbolKellyHalfFraction, 1, currentMarketHealth);
...
//                                    var direction = (signal == SignalDecision.Buy) ? PositionDirection.Long : PositionDirection.Short;
//                                    decimal effectiveEntryPrice = transactionCostService.CalculateEntryCost(rawEntryPrice, signal, symbol, interval, currentBar.Timestamp);
//                                    decimal totalEntryCost = transactionCostService.GetSpreadCost(rawEntryPrice, quantity, symbol, interval, currentBar.Timestamp);
//                                    decimal totalCashOutlay = (quantity * rawEntryPrice) + totalEntryCost;
...
//                                        {
//                                            decimal entryAtr = currentIndicatorValues["ATR"];
...
//                            var lastIndicators = new Dictionary<string, decimal>
//                            {
...
//                decimal finalEquity = _portfolioManager.GetTotalEquity();
...
//            var allTrades = new ConcurrentBag<TradeSummary>();
//            var riskAssetKellyHalfFractions = new ConcurrentDictionary<string, decimal>();
...
//                var assetDataCache = new Dictionary<string, List<PriceModel>>();
...
//            await _portfolioManager.Initialize(config.SessionId.Value, config.InitialCapital);
//            var allTrades = new ConcurrentBag<TradeSummary>();
//            var pairKellyHalfFractions = new ConcurrentDictionary<string, decimal>();
...
//                    var closePricesA = alignedData.Select(d => (decimal)d.Item1.ClosePrice).ToArray();
//                    var closePricesB = alignedData.Select(d => (decimal)d.Item2.ClosePrice).ToArray();

//                    // 1. Calculate the entire spread series
//                    var spreadSeries = new decimal[alignedData.Count];
...
//                    var zScoreSeries = new decimal[alignedData.Count];
//                    for (int i = 0; i < alignedData.Count; i++)
//                    {
//                        if (spreadStdDev[i] != 0)
...
//                    decimal currentPairKellyHalfFraction = pairKellyHalfFractions.GetOrAdd(pairIdentifier, 0.01m);
...
//                        if (activePairTrade != null && strategyInstance.ShouldExitPosition(new Models.Trading.Position { Direction = activePairTrade.Direction, EntryDate = activePairTrade.EntryDate }, currentBarA, currentBarB, currentIndicatorValues))
//                        {
...
//                            var effectiveExitPriceA = _transactionCostService.CalculateExitCost(rawExitPriceA, directionA, pair.SymbolA, config.Interval, currentBarA.Timestamp);
//                            var exitSpreadCostA = _transactionCostService.GetSpreadCost(rawExitPriceA, (int)activePairTrade.QuantityA, pair.SymbolA, config.Interval, currentBarA.Timestamp);
...
//                            var effectiveExitPriceB = _transactionCostService.CalculateExitCost(rawExitPriceB, directionB, pair.SymbolB, config.Interval, currentBarB.Timestamp);
//                            var exitSpreadCostB = _transactionCostService.GetSpreadCost(rawExitPriceB, (int)activePairTrade.QuantityB, pair.SymbolB, config.Interval, currentBarB.Timestamp);
...
//                            decimal allocation = _portfolioManager.GetTotalEquity() * currentPairKellyHalfFraction;
...
//                            var effectiveEntryPriceA = _transactionCostService.CalculateEntryCost(currentBarA.ClosePrice, signal, pair.SymbolA, config.Interval, currentBarA.Timestamp);
...
//                            var effectiveEntryPriceB = _transactionCostService.CalculateEntryCost(currentBarB.ClosePrice, signal, pair.SymbolB, config.Interval, currentBarB.Timestamp);
...
//                            decimal totalCostToOpen = (quantityA * effectiveEntryPriceA) + (quantityB * effectiveEntryPriceB) + entrySpreadCostA + entrySpreadCostB;
...
//        private List<(PriceModel, PriceModel)> AlignData(
//            IEnumerable<PriceModel> dataA,
//            IEnumerable<PriceModel> dataB)
//        {
//            if (dataA == null || dataB == null)
//                return new List<(PriceModel, PriceModel)>();
